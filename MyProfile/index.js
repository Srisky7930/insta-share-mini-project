...
class MyProfile extends Component {
  ...
  getMyProfile = async () => {
    ...
    if (response.ok === true) {
      const fetchedData = data.profile
      const myProfileData = {
  ...
  getProfile = () => {
    const {myProfile, posts, stories} = this.state
    const {
      userName,
      profilePic,
      postsCount,
    ...
    return (
      <div className="my-profile-main-container">
        <div className="container">
          ...
          <div>
            <h1 className="story-highlights">Story Highlights </h1>
          </div>

          <div>
            <ul className="stories-list">
              {stories.map(each => (
                <li className="stories-items" key={each.id}>
                  <img
                    src={each.image}
                    alt="my story"
          ...
          <div>
            ...
            {showPosts ? (
              <ul className="posts-list">
                {posts.map(each => (
                  <li className="posts-items" key={each.id}>
                    <img
                      src={each.image}
                      alt="my post"
  ...
  renderProfileDetails = () => {
    ...
    switch (apiStatus) {
      ...
      case apiStatusConstants.inProgress:
        return this.getProfileLoaderView()
